...
class _QueryProxy(flask_sqlalchemy.BaseQuery):
    ...
    #def __init__(self, query_obj, primary_key_name, whoosh_searcher, model):  #delete by feng.guanhua
    # Make this a pure copy of the original Query object.
    #   self.__dict__ = query_obj.__dict__.copy()               #delete by feng.guanhua
    #   self._primary_key_name = primary_key_name               #delete by feng.guanhua
    #   self._whoosh_searcher = whoosh_searcher                 #delete by feng.guanhua
    #   self._modelclass = model                                #delete by feng.guanhua
    # Stores whoosh results from query. If ``None``, indicates that no
    # whoosh query was performed.
    #   self._whoosh_rank = None                                #delete by feng.guanhua
...
def _create_index(app, model):
    ...
    #searcher = _Searcher(primary_key, indx)                    #delete by feng.guanhua
    #model.query = _QueryProxy(model.query, primary_key,        #delete by feng.guanhua
    #        searcher, model)                                   #delete by feng.guanhua
    #model.pure_whoosh = searcher

    model.pure_whoosh = _Searcher(primary_key, indx)            #add by feng.guanhua
    model.whoosh_primary_key = primary_key                      #add by feng.guanhua
    # change the query class of this model to our own           #add by feng.guanhua
    model.query_class = _QueryProxy                             #add by feng.guanhua
